...
export default async function FreshwaterFishPage() {
  const response = await fetch('http://localhost:8080/api/categories/cat?category=freshwater fish', { cache: 'no-store' });
  ...
  return (
    <div>
      <div className="bg-red-600 text-white p-6 flex flex-col md:flex-row items-center justify-center gap-4">
        <h1 className="text-4xl font-bold text-center">Freshwater Fish on Sale!</h1>
        <button className="bg-yellow-400 text-black font-semibold py-2 px-4 rounded shadow-md hover:bg-yellow-500">
          Shop now ➔
        </button>
      </div>
      <div className="px-6 py-4">
        <h2 className="text-3xl font-bold text-gray-800">Freshwater Fish</h2>
        <p className="text-gray-700 mt-2">
          No one offers a better selection of healthy Freshwater fish for the Freshwater aquarium
          than FishStore®. From  and more... The finest specimens
          in the world are just a click away. Whether you’re a beginner or a seasoned hobbyist, find quality aquatic life for Freshwater aquarium when you shop FishStore®.
      ...
      <div className="grid grid-cols-2 gap-8 p-6 max-w-4xl mx-auto">
        {subcategories.map(({ subcategory }: Subcategory) => (
          <div key={subcategory} className="border rounded-lg p-4 shadow-md text-center">
            <Link href={`/category/freshwater-fish/${subcategory.replace('-', ' ')}`}>
              <div>
                <Image
